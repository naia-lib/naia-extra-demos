...
use multi_client_server_c_protocol::{protocol, Auth, StringMessage};
...
impl App {
    pub fn new() -> Self {
        info!("Multi-Client Server C started");
        ...
        if events.is_empty() {
            ...
            for _ in events.read::<TickEvent>() {
                ...
                for user_key in self.server.user_keys() {
                    let new_message_contents = format!("Server C Message ({})", self.tick_count);
                    info!(
                        "Server C send to   ({}) -> {}",
                        self.server.user(&user_key).address(),
